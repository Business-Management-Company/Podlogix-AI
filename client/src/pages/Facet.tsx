    ...
    try {
      ...
      try {
        ...
        if (selected.type === "video" && haveSpans) {
          ...
          const videoCmd = `ffmpeg -y -i {input} -vf "select='${expr}',setpts=N/FRAME_RATE/TB${eq},scale=-2:min(720\\,ih)" -af "aselect='${expr}',asetpts=N/SR/TB,loudnorm=I=-16:TP=-1.5:LRA=11" -c:v h264_nvenc -preset p1 -cq 23 -b:v 2500k -maxrate 3500k -bufsize 7M -c:a aac -b:a 160k {output}`;
          try {
          ...
          const cmd = colorCorrect
            ? `ffmpeg -y -i {input} -vf eq=brightness=0.02:contrast=1.05:saturation=1.1 -c:v h264_nvenc -preset p1 -cq 23 -b:v 2500k -maxrate 3500k -bufsize 7M -af loudnorm=I=-16:TP=-1.5:LRA=11 -c:a aac -b:a 160k {output}`
