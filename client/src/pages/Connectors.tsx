...
export default function Connectors() {
  ...
  const connectSpotifyMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("GET", "/api/spotify/auth-url");
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: phylloStatus?.configured
          ? "Social account is now being monitored."
          : "Demo account added. Configure Phyllo API for real monitoring.",
      });
    },
    onError: (error: Error) => {
          ...
          <CardContent>
            {spotifyStatus?.isConnected ? (
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">{spotifyStatus.displayName}</p>
                  <p className="text-xs text-muted-foreground">{spotifyStatus.email}</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <div>
                ...
                <CardDescription>
                  {phylloStatus?.configured
                    ...
                    : "Demo mode active. Configure Phyllo API keys for real social monitoring."}
                </CardDescription>
              </div>
            </div>
          </CardHeader>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Overview of configured external services</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
