...
- **Content** flows through `lib/content.ts`: the app's public feed first (featured Podlogix shows, then Podchaser's most followed shows and their hosts, fetched by the app with its own key), then seed data from `lib/data.ts`, cached for an hour. Sections always receive five trending shows and ten creators. The category rail reads the app's categories feed (labels and real show counts) and dresses each card with the top non explicit show cover from Apple's chart for that genre, cached for a day, with a full snapshot baked into the seed.
